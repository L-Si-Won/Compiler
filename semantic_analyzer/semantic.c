...
int sem_declaration(A_ID *id,int addr)
{
	...
	switch (id->kind) {
		...
		case ID_FIELD:
			...
			if (i == 0) //self-referential structure
				semantic_error(87,id->line);
			if (i%4)
				i=i/4*4+4; 
			id->address=addr;
			size=i;
...
void semantic_error(int i, int ll, char *s) {
	semantic_err++;
	printf("*** semantic error %d at line %d: ", i, ll);
	switch (i){
	// errors in expression
		...
		case 18:printf("illegal constant expression\n");
		      break;
		case 19:printf("illegal identifier %s constant expression\n", s);
		      break;
		...
		case 29:printf("integral type expression required in expression\n");
		      break;
		case 31:printf("pointer typer expr required in pointer operation\n");
		      break;
	...
	// errors in statement
		...
		case 72:printf("default label not within a switch statement\n");
		      break;
		case 73:printf("break statement not within loop or switch statement\n");
		      break;
	...
	// errors in type & declarator
		...
		case 86:printf("illegal array size or empty array\n");
		      break;
		case 87:printf("self-referential structure\n");
		      break;
